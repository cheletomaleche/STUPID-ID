...
const App: React.FC = () => {
  ...
  const handleApplyStyle = (option: StylingOption) => { processEdit(option.prompt); };
  const handleFaceEdit = (prompt: string) => { processEdit(prompt); };
  ...
  const checkCompliance = async () => {
    ...
    try {
      ...
      setState(prev => ({ ...prev, compliance: result, processingState: ProcessingState.IDLE }));
    } catch (error) {
      setState(prev => ({ ...prev, processingState: ProcessingState.ERROR }));
    }
  };
  ...
  return (
    <div className="h-screen w-full bg-grey-900 text-slate-200 relative overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex flex-col relative animate-in fade-in duration-500">
           ...
           <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-8">
              ...
              <div className="space-y-4 w-full max-w-xs">
                   <button onClick={() => fileInputRef.current?.click()} className="w-full py-4 bg-gold-500 hover:bg-gold-400 text-grey-900 font-bold rounded-xl flex items-center justify-center gap-3 transition-transform active:scale-95 shadow-lg">
         ...
         <div className="flex-1 flex flex-col relative animate-in slide-in-from-right duration-300">
            ...
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center space-y-8">
                ...
                <div className="flex flex-col gap-4 w-full max-w-xs mt-8">
                  <button onClick={() => selectGender(Gender.MALE)} className="w-full py-6 bg-grey-800 hover:bg-grey-700 hover:border-gold-400 border border-grey-700 rounded-xl flex items-center justify-center gap-4 transition-all group">
                     <div className="w-12 h-12 rounded-full bg-grey-900 flex items-center justify-center group-hover:bg-gold-400 group-hover:text-grey-900 transition-colors"><UserCircle size={28} /></div>
                     <div className="text-left"><span className="block text-lg font-bold text-white">Male Styles</span><span className="text-xs text-slate-400">Suits, Short Hair, Beards</span></div>
                  </button>
                  <button onClick={() => selectGender(Gender.FEMALE)} className="w-full py-6 bg-grey-800 hover:bg-grey-700 hover:border-gold-400 border border-grey-700 rounded-xl flex items-center justify-center gap-4 transition-all group">
                     <div className="w-12 h-12 rounded-full bg-grey-900 flex items-center justify-center group-hover:bg-gold-400 group-hover:text-grey-900 transition-colors"><UserCircle size={28} /></div>
                     <div className="text-left"><span className="block text-lg font-bold text-white">Female Styles</span><span className="text-xs text-slate-400">Blazers, Long Hair, Makeup</span></div>
                  </button>
                </div>
            </div>
         </div>
        ...
        <div className="flex-1 flex flex-col h-full animate-in fade-in duration-300">
          <header className="h-16 flex items-center justify-between px-4 border-b border-grey-700 bg-grey-900 z-20 flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
              <button onClick={handleStartOver} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-grey-800 text-slate-300 hover:text-gold-400 hover:bg-grey-700 transition-all border border-transparent hover:border-gold-400/30 mr-2" title="Upload New Photo">
                <ImagePlus size={18} /> <span className="hidden sm:inline text-xs font-semibold">New</span>
              </button>
              <div className="h-6 w-px bg-grey-700 mx-1"></div>
              <button disabled={state.historyIndex <= 0} onClick={handleUndo} className="p-2 text-slate-400 hover:text-white disabled:opacity-30 disabled:hover:text-slate-400 transition-colors" title="Undo"><Undo2 size={20} /></button>
              <button onClick={checkCompliance} className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-grey-800 border border-grey-700 text-xs font-semibold text-slate-300 hover:border-gold-400 hover:text-gold-400 transition-all"><ShieldCheck size={14} /> Check</button>
              <button onClick={() => setIsExportOpen(true)} className="p-2 text-gold-400 hover:text-gold-300 transition-colors" title="Download"><Download size={22} /></button>
            </div>
          </header>
          ...
          <main className="flex-1 relative overflow-hidden bg-grey-950 flex items-center justify-center p-4 sm:p-8">
            <div className="relative shadow-2xl rounded-sm overflow-hidden border-4 border-white aspect-[3/4] h-full max-h-[500px] sm:max-h-[600px] w-auto">
                 <img src={state.currentImage} alt="ID Preview" className="w-full h-full object-cover" />
                 {(state.processingState === ProcessingState.PROCESSING || state.processingState === ProcessingState.CHECKING_COMPLIANCE) && (
                    <div className="absolute inset-0 bg-grey-900/80 backdrop-blur-sm flex flex-col items-center justify-center gap-4 z-10">
                        ...
                        <p className="text-gold-400 font-medium animate-pulse">{state.processingState === ProcessingState.PROCESSING ? 'Styling AI is working...' : 'Analyzing Compliance...'}</p>
                    </div>
                 )}
            </div>
            <button onClick={handleReset} className="absolute top-4 right-4 p-2 bg-grey-800/80 rounded-full text-slate-400 hover:text-white backdrop-blur-md" title="Reset to Original"><RotateCcw size={16} /></button>
          </main>

          <div className="bg-grey-900 border-t border-grey-800 flex flex-col flex-shrink-0">
            <div className="h-44 px-4 py-4">
                 <ControlPanel activeTab={state.activeTab} onApplyStyle={handleApplyStyle} onApplyFaceEdit={handleFaceEdit} isProcessing={state.processingState !== ProcessingState.IDLE} selectedGender={state.gender} />
            </div>
            <nav className="h-20 border-t border-grey-800 flex items-center justify-around px-2 pb-safe bg-grey-900">
                {TAB_CONFIG.map((tab) => (
                    <button key={tab.id} onClick={() => setState(prev => ({ ...prev, activeTab: tab.id }))} className={`flex flex-col items-center justify-center gap-1 w-16 h-16 rounded-2xl transition-all duration-300 ${state.activeTab === tab.id ? 'text-grey-900 bg-gold-400 shadow-[0_0_15px_rgba(204,164,59,0.4)] transform -translate-y-2' : 'text-slate-500 hover:text-slate-300'}`}>
      ...
      {state.compliance && <ComplianceModal result={state.compliance} onClose={() => setState(prev => ({ ...prev, compliance: null }))} />}
      {isExportOpen && <ExportModal imageBase64={state.currentImage} onClose={() => setIsExportOpen(false)} onReset={handleStartOver} />}
    </div>
  );
};
